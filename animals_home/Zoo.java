...
public class Zoo {
    ...
    private List<Floating> floatings (){
        List<Floating> res = new ArrayList<>();
        for (Animal animal:animals) {
            if (animal instanceof Floating) {
                res.add((Floating) animal);
            }
        }
        return res;
    }

    public Animal championByFloating () {
        List<Floating> start = floatings();
        Floating champ = start.get(0);
        for (Floating swimmer:start) {
